// Testbench: CV32E40X + CV-X-IF coprocessor running a real RISC-V program.
//
...
module tb_e40x;
      ...
      if (data_req & data_gnt) begin
        if (data_addr == RESULT_ADDR && data_we && !RESULT_IN_MEM) begin
          if (n_results >= N_GOLDEN) begin
            errors++;
            $error("result #%0d %08x: more results than expected (%0d)",
                   n_results, data_wdata, N_GOLDEN);
          end else if (data_wdata !== GOLDEN[n_results]) begin
            errors++;
            $error("result #%0d = %08x, expected %08x",
  ...
  always @(posedge clk) if (rst_n) begin
    if (dut.xif.issue_valid && dut.xif.issue_ready)
      begin if (dut.xif.issue_resp.accept) n_offload++; else n_reject++; end
    if (dut.xif.commit_valid && dut.xif.commit.commit_kill) n_kill++;
    if (dut.xif.result_valid && dut.xif.result_ready) n_xresult++;
    if (dut.xif.issue_valid && !dut.xif.issue_ready) n_stallcyc++;
  end
  ...
  initial begin
    ...
    fork
      ...
      begin
        ...
        $error("timeout: program did not signal completion");
        errors++;
      end
    join_any
    disable fork;
    ...
    if (RESULT_IN_MEM) begin
      for (int i = 0; i < N_GOLDEN; i++) begin
        if (mem[(RESULT_BASE >> 2) + i] !== GOLDEN[i]) begin
          errors++;
          // $display rather than $error: Verilator stops the simulation on the
          // first $error, so a run with several mismatching words reported one
          // and hid the rest. The run still fails -- errors is checked below --
          // but the whole list is printed, which is what a caller reading
          // values back out of the checker needs.
    ...
    if (n_offload == 0) begin errors++; $error("no instruction was ever offloaded!"); end
    if (n_kill == 0)    begin errors++; $error("commit_kill path never exercised!"); end
    if (n_reject == 0)  begin errors++; $error("reject path never exercised!"); end
